...
def evaluate_explainability(topk_matches, test_user_products):
...
def evaluate_paths(path_file, train_labels, test_labels):
    ...
    for uid in best_pred_paths:
        ...
        top10_pids_path = [(p[-1][2], str(p[0][1]) + ' ' + str(p[0][2]) + ' has ' + str(p[1][0]) + ' ' + str(p[1][1]) + ' ' + str(p[1][2]) + ' which was ' + str(p[2][0]) + ' by ' + str(p[2][1]) + ' ' + str(p[2][2]) + ' who ' + str(p[3][0]) + ' ' + str(p[3][1]) + ' ' + str(p[3][2])) for _, _, p in sorted_path[:10]]  # from largest to smallest
    ...
    for i in pred_labels_1[:len(pred_labels_1) - 5:-1]:
        print('pred_labels[uid] :', i, pred_labels_path[i])
