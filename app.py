...
def main(args, logger):
    ...
    selected_user_key = st.selectbox('Select a user:', list(test_labels.keys()))
    ...
    with p_tab1:
        st.subheader("Prior purchases of user: " + str(selected_user_name))
        p_col1, p_col2, p_col3 = st.columns([1,1,4])
        p_col1.markdown("#### Product Name")
        p_col2.markdown("#### Brand")
        p_col3.markdown("#### Category")
        for prod in train_labels[args.users]:
            p_col1.write(get_entity_details(dataset, "product", prod))
            p_col2.write(str([get_entity_details(dataset, "brand", brand) for brand in dataset.produced_by.data[prod]]))
            p_col3.write(str([get_entity_details(dataset, "category", cat) for cat in dataset.belongs_to.data[prod]]))
    with p_tab2: 
    ...
    if recommend_button:
        ...
        if pred_labels:
            ...
            with tab1:
                st.subheader("Recommended Products with basic recommendation features")
            with tab2:
                st.subheader("Recommended Products with explainability features")
                # Display posters in a grid layout
                col1, col2, col3 = st.columns([1,3,1])
                col1.markdown("#### Product")
                col2.markdown("#### Explainability of the recommendation")
                col3.markdown("#### Max Explainability Score")
                for user in pred_labels_path:
                    for prod_path in pred_labels_path[user]:
                        col1.write(prod_path[0])
                        col2.write(prod_path[1])
            with tab3:
                st.subheader("Recommended Products powered by Max Explainability Score (MES) features")
            with tab4:
